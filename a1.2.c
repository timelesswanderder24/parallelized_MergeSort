...
void *merge_sort_left(void *data) {
...
int main(int argc, char *argv[]) {
    ...
    printf("starting---\n");
    struct left_start_block;
    struct right_start_block;
    struct *right_part = right_start_block;
    left_start_block.size = size/2;
    left_start_block.first = data;
    right_start_block.size = left_start_block.size + (size%2);
    right_start_block.first = data + left_start_block.size;
    merge_sort(&left_start_block);   //address where the start_block starts, will return a pointer
    pthread_create(&right_thread, NULL, merge_sort_left, (void*)right_part);
