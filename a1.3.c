...
void *merge_sort_thread(void *data) {
    ...
    if (my_data->size > 1) {
        ...
        right_block.size = left_block.size + (my_data->size % 2);
        right_block.first = my_data->first + left_block.size; // add the first element pointer to the size of left block so that the pointer will move to the middle memory location
        merge_sort_thread(&left_block);
        merge_sort_thread(&right_block);
        merge(&left_block, &right_block);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    merge_sort_thread(&left_block1);
    pthread_join(right_thread1, NULL);
    pthread_join(left_thread2, NULL);
    pthread_join(right_thread2, NULL);
    merge(&left_block1, &right_block1);
    struct block left_main;
    left_main.size = left_block1.size + right_block1.size;
    left_main.first = left_block1.first;
    merge(&left_block2, &right_block2);
    struct block right_main;
    right_main.size = left_block2.size + right_block2.size;
    right_main.first = left_block2.first;
    merge(&left_main, &right_main);
